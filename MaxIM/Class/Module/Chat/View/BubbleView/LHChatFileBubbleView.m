...
//　textLaebl 最大宽度
CGFloat const FILETEXTLABEL_MAX_WIDTH = 200.0f;
CGFloat const FILELABEL_FONT_SIZE = 15.0f;

static CGSize kFileTextBoundingSize;

@interface LHChatFileBubbleView () {
...
@implementation LHChatFileBubbleView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        _textLabel.font = [UIFont systemFontOfSize:FILELABEL_FONT_SIZE];
...
-(void)layoutSubviews {
    ...
    frame.size.width  = MAXScreenW / 2.0 - 50 ;
    ...
    self.fileImageView.frame = CGRectMake(self.textLabel.right + 3 , CGRectGetMinY(self.textLabel.frame), 40, 40);
}
...
- (void)setMessageModel:(LHMessageModel *)messageModel {
    ...
    NSAttributedString *text = [LHChatFileBubbleView processModel:self.messageModel];
    YYTextContainer *container = [YYTextContainer containerWithSize:CGSizeMake(FILETEXTLABEL_MAX_WIDTH, MAXFLOAT)];
    ...
    kFileTextBoundingSize = textLayout.textBoundingSize;
...
+ (NSAttributedString *)processModel:(LHMessageModel *)model {
    ...
    if (!regularExpression) {
        ...
        while (index > 0) {
            ...
            if (imageName) {
                //获取图片
                YYImage * image = [LHChatFileBubbleView getImageWithRangeString:imageName];//这是个自定义的方法
                if (image != nil) {
...
- (CGSize)sizeThatFits:(CGSize)size {
    CGFloat height = 40;
    if (2*BUBBLE_VIEW_PADDING + kFileTextBoundingSize.height > height) {
        height = 2*BUBBLE_VIEW_PADDING + kFileTextBoundingSize.height;
    }
    
    
    CGFloat width = kFileTextBoundingSize.width + BUBBLE_VIEW_PADDING*2 + BUBBLE_VIEW_PADDING;
    if (width < 46.5) {
        width = 46.5;
    }
    
    return CGSizeMake(MAXScreenW/2.0, height + 20);
}
...
+ (UIFont *)textLabelFont {
    return [UIFont systemFontOfSize:FILELABEL_FONT_SIZE];
}
...
- (UIImageView *)fileImageView {
    if (_fileImageView == nil) {
        ...
        //        _fileImageView.backgroundColor = [UIColor yellowColor];
        [self addSubview:_fileImageView];
