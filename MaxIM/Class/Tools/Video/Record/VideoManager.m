...
- (void)startRecordingVideoWithFileName:(NSString *)videoName
{
    ...
    if (!connection) {
        MAXLog(@"capture connection wrong!");
        return;
    }
...
- (int)getVideoTimeByUrlString:(NSString*)urlString {
    ...
    if (urlString && ![urlString isKindOfClass:[NSNull class]] && [[NSFileManager defaultManager] fileExistsAtPath:urlString]) {
        MAXLog(@"you");
    }
    NSURL*videoUrl = [NSURL fileURLWithPath:urlString];
    AVURLAsset *avUrl = [AVURLAsset assetWithURL:videoUrl];
    CMTime time = [avUrl duration];
    int audioDurationSeconds= CMTimeGetSeconds(time);
...
- (CGFloat)getFileSize:(NSString *)path
{
    ...
    MAXLog(@"file size : %f",[outputFileAttributes fileSize]/1024.0/1024.0);
...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error
{
    ...
    [self compressVideo:[mp4Url path] finished:^(NSString *path) {
        ...
        // 删除原录制的文件
        if ([VideoTool fileExistsAtPath:outputFileURL.path]) {
            [VideoTool removeFileAtPath:outputFileURL.path];
        }
    }];
    // 直接转大小不会改变
...
{
    NSString *videoPath   = [[VideoManager shareManager] videoPathWithFileName:[[namePath lastPathComponent] stringByDeletingPathExtension]];
